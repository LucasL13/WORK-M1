/** La classe contenant les fonctions utilitaires pour le cryptage/décryptage AES codées en TP */
/** Construit la clée étendue à partir de la clée courte **/

public class Diversification {

    public static boolean debug = false;

    static int[] SBox = { };


    static int[] Rcon = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36 };

    public int[][] W;
    public int[][] K;


    static int Nr;
    static int Nk;


    public void affiche_la_clef(){
        int x = 0;
        for (int i=0; i < W.length; i++){
            for(int j=0; j < W[i].length; j++) {
                System.out.format("%02X ", W[i][j]);
                x++;
                if(x % 16 == 0)
                    System.out.println();
            }
        }
        System.out.println();
    }

    public int[] RotWord(int[] tmp){
        int[] ttmp = new int[tmp.length];
        for(int i=0; i < tmp.length; i++) {
            ttmp[i] = tmp[((i + 1) % tmp.length)];
        }
        return ttmp;
    }

    public int[] SubWord(int[] tmp){
        int[] ttmp = new int[tmp.length];
        for(int i=0; i < tmp.length; i++)
            ttmp[i] = SBox[tmp[i]];
        return ttmp;
    }

    public void calcule_la_clef_etendue(){

        switch ((K.length*4)){
            case 16: Nr = 10; Nk = 4; break;
            case 24: Nr = 12; Nk = 6; break;
            case 32: Nr = 14; Nk = 8; break;
            default:
                System.out.println("Erreur taille de la clée courte invalide\n");
        }

        W = new int[(4 * (Nr+1))][4];

        if(debug){
            System.out.println("K = " + K.length *4 );
            System.out.println("Nr = " +Nr);
            System.out.println("Nk = " + Nk);
            System.out.println("W = " + W.length*4);
            System.out.println();
        }

        // Copie de W dans K
        for(int i=0; i < Nk; i++)
            for(int j=0; j < 4; j++)
                W[i][j] = K[i][j];


        // Debut du calcul des nouvelles colones
        for(int i = Nk; i < (4 * (Nr+1)); i++){

            // Initialisation du vecteur tmp (creation+copie depuis la colone en cours de W)
            int tmp[] = new int[4];
            for(int j =0; j < 4; j++)
                tmp[j] = W[i-1][j];

            //System.out.println("Tmp=["+tmp[0]+" "+tmp[1]+" "+tmp[2]+" "+tmp[3]+"]");

            // Debut des opérations
            if((i % Nk) == 0){
                tmp = RotWord(tmp);
                tmp = SubWord(tmp);
                tmp[0] = tmp[0] ^ Rcon[((i/Nk)-1)];
            }
            else if((Nk > 6) && (i % Nk == 4)){
                tmp = SubWord(tmp);
            }

            // XOR tmp ° W[i-Nk]
            for(int j =0; j < 4; j++)
                tmp[j] = W[i - Nk][j] ^ tmp[j];

            // Copie du vecteur tmp calculé dans la colone en cours de W
            for(int j=0; j < 4; j++)
                W[i][j] = tmp[j];
        }
    }

}
