...
public abstract class ReseauxToolbox {
    ...
    // Une méthode pour faciliter la réception d'un message par les classes héritées
    public String getMessage(Socket soc){
        String message = "";
        try{
            BufferedReader brGet = new BufferedReader(new InputStreamReader(soc.getInputStream()));
            while(brGet.ready())
                message += (char) brGet.read();
        }
